...
func runSyncPRProject(ctx logx.LogCtx, repoRoot AbsPath, project string, base string) error {
	...
	headSHA, err := cmdx.New("git", "rev-parse", "origin/"+syncBranch).In(repoRoot).
		Run(ctx, cmdx.RunOptions{CaptureStdout: true, TransformEnv: nil})
	if err != nil {
		return err
	}
...
func listOpenPRs(
	...
	out, err := listPRsCmd.Run(ctx, cmdx.RunOptions{CaptureStdout: true, TransformEnv: nil})
	if err != nil {
		return nil, err
	}
...
func ensureLabelExists(
	...
	out, err := listLabelsCmd.Run(ctx, cmdx.RunOptions{CaptureStdout: true, TransformEnv: nil})
	if err != nil {
		return err
	}
	var labels []struct {
