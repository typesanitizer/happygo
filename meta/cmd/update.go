...
func (ws Workspace) runUpdate(ctx logx.LogCtx, dir string, localBranch string, projects []string) error {
	for _, project := range projects {
		...
		stdout, err := subtreePullCmd.Run(ctx, cmdx.RunOptions{CaptureStdout: true, Env: nil})
		if err != nil {
			if stdout != "" {
